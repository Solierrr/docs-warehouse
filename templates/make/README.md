...
ficam em [`Solierrr/org-scripts`](https://github.com/Solierrr/org-scripts),
...
Em ambientes Unix, o diretório equivalente é `~/.local/share/solierrr-org-scripts`.
...
```powershell
make tools-check ORG_SCRIPTS_DIR=C:/ferramentas/org-scripts
```

## Alvos organizacionais obrigatórios

- `make help`: lista os comandos disponíveis.
- `make tools-check`: confirma que `org-scripts` está instalado e contém o
  script chamado pelo repositório.
- `make env ENV=local`: gera o arquivo de ambiente por meio de
  `org-scripts`. O projeto deve informar seu `SERVICE` padrão; `OUT` é
  opcional e não deve apontar para arquivo versionado.

O alvo `env` nunca executa login, instala CLIs ou atualiza `org-scripts`
...
`org-scripts` é a fonte única para utilitários que atendem vários repositórios,
